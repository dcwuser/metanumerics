    ...
    public class ContinuousDistributionTest {
        ...
        private static List<ContinuousDistribution> CreateDistributions () {
            ...
            List<ContinuousDistribution> distributions = new List<ContinuousDistribution>(new ContinuousDistribution[] {
                ...
                new SkewNormalDistribution(1.0),
        ...
        public void DistributionMeanIntegral () {
            foreach (ContinuousDistribution distribution in distributions) {
                ...
                if (distribution is BetaDistribution b && ((b.Alpha < 1.0) || (b.Beta < 1.0))) continue;
                double M1 = FunctionMath.Integrate((double x) => distribution.ProbabilityDensity(x) * x, distribution.Support);
                if (mean == 0.0) {
        ...
        public void DistributionVarianceIntegral () {
            foreach (ContinuousDistribution distribution in distributions) {
                ...
                if ((distribution is GammaDistribution g) && (g.Shape < 1.0)) e = Math.Sqrt(e);
                double m = distribution.Mean;
                double C2 = FunctionMath.Integrate((double x) => distribution.ProbabilityDensity(x) * MoreMath.Sqr(x - m), distribution.Support, new IntegrationSettings() { RelativePrecision = e, AbsolutePrecision = 0.0 }).Value;
        ...
        public void DistributionRawMomentIntegral () {
            foreach (ContinuousDistribution distribution in distributions) {
                foreach (int r in TestUtilities.GenerateIntegerValues(2, 32, 8)) {
                    ...
                    try {
                        IntegrationResult MI = FunctionMath.Integrate((double x) => distribution.ProbabilityDensity(x) * MoreMath.Pow(x, r), distribution.Support);
        ...
        public void DistributionInvalidProbabilityInput () {
            foreach (ContinuousDistribution distribution in distributions) {
                Assert.ThrowsException<ArgumentOutOfRangeException>(() => distribution.InverseLeftProbability(1.1));
                Assert.ThrowsException<ArgumentOutOfRangeException>(() => distribution.InverseLeftProbability(-0.1));
                Assert.ThrowsException<ArgumentOutOfRangeException>(() => distribution.InverseRightProbability(1.1));
                Assert.ThrowsException<ArgumentOutOfRangeException>(() => distribution.InverseRightProbability(-0.1));
            }
        }
        ...
        public void GammaFromExponential () {
            ...
            foreach (int n in new int[] { 2, 3, 4, 5 }) {
                List<double> gSample = new List<double>();
                for (int i = 0; i < 100; i++) {
        ...
        public void DistributionRandomDeviates () {
            ...
            foreach (ContinuousDistribution distribution in distributions) {
                List<double> s = distribution.GetRandomValues(rng, 128).ToList();
                TestResult r = s.KolmogorovSmirnovTest(distribution);
                Assert.IsTrue(r.Probability > 0.005);
