...
namespace Meta.Numerics.Matrices {
    ...
    public abstract class AnyVector : AnyRectangularMatrix, IEnumerable, IEnumerable<double>, ICollection<double>, IList<double>, IReadOnlyCollection<double>, IReadOnlyList<double> {

        // Should seriously consider making this just implement read-only interfaces.
        // Most of the write interface functionality is not supported or very corner-case.
        // Only advantage of keeping I can see is some APIs still coded to write interfaces even though they probably don't have to be,
        // like e.g. constructor for read-only collection.

        internal AnyVector (double[] store, int offset, int stride, int dimension, bool isReadOnly) : base(isReadOnly) {
            this.store = store;
            this.offset = offset;
            this.stride = stride;
        ...
        internal AnyVector (double[] store, int dimension, bool isReadOnly) : this(store, 0, 1, dimension, isReadOnly) { }

        internal AnyVector (double[] store, int dimension) : this(store, dimension, false) { }

        internal AnyVector (int dimension) : this(new double[dimension], dimension) { }

        internal AnyVector (IReadOnlyList<double> list) {
